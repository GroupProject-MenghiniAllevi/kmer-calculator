...
class OutputWriter:
    ...
    def write_to_output(self, kmer_dictionary):
        ...
        if file_lenght == 0:
            ...
            kmer_keys = self.__get_keys_from_dict(kmer_dictionary)
            kmer_duplicates_indexs = self.get_index_of_changed_value(kmer_keys, new_kmer_indexs)
            ...
            self.get_row_length()
            kmer_duplicates_values = self.get_dupl_kmer_value_from_dict(kmer_dictionary, kmer_duplicates_indexs)
            kmer_new_values = list()
            for key in kmer_new_keys:
    ...
    def __get_keys_from_dict(self, kmer_dict):
        """
        Questo metodo permette di restituire i kmer presenti nella chiave del dict
        :param kmer_dict: un dict formato da chiave che rappresenta il kmer e il valore rappresenta il conteggio.
        :return: una lista contenente i kmer presenti nel dict
        """
        l = list()
    ...
    def read_and_write_intestation(self, kmer_list):
        ...
        new_out_index_column = 0
        while True:
            c = self.__out_file.read(1)
            if not c or c == b'\n' or c == b'\r':
                if index_column == 0:
                    self.__new_out_file.write(b'id\n')
                    new_out_index_column += 1
                elif len(list_new_kmer) - 1 >= index_list and previous_readed < list_new_kmer[
                    index_list] < readed_value and len(previous_readed) <= len(list_new_kmer[index_list]) <= len(
                        readed_value):
                    s = list_new_kmer[index_list] + ","
                    index_new_kmer.append(new_out_index_column)
                    self.__new_out_file.write(s.encode())
                    if index_list <= len(list_new_kmer) - 1:
                    ...
                    if index_list <= len(list_new_kmer) - 1:
                        ...
                        index_column += 1
                        index_new_kmer.extend(
                            self.__add_remain_kmer_to_end(list_new_kmer, index_list, new_out_index_column))
                    else:
                        s = readed_value + "\n"
                        self.__new_out_file.write(s.encode())
                    new_out_index_column += 1
    ...
    def get_index_of_changed_value(self, kmer_key_values, new_kmer_index):
        """
        Questo metodo permette di calcolare l'indice di ogni valore già presente nel file di uscita, utilizzando
        il file di out temporaneo, così che l'indice corrisponda al nuovo file di uscita.
        :param kmer_key_values: la lista totale di tutti i kmer da aggiungere.
        :param new_kmer_index: l'indice dei nuovi kmer.
        :return: una lista contenente gli indici dei kmer già presenti.
        """
        list_of_index = list()
        with open(self.__new_out_path, "rb") as file:
            ...
            while True:
                ...
                if not c or c == b"\r" or c == b"\n":
                    if readed_value in kmer_key_values and not column_index in new_kmer_index:
            ...
            if c == b"," or c == b"\n" or c == b"\r" or not c:
                ...
                if actual_filename == self.__file_name:
                    if index_column in new_kmer_indexs:
                        value = str(kmer_new_values[index_new_value])
                        index_new_value += 1
                        self.__out_file.seek(-(len(readed_value)+1),1)
                    elif index_column in kmer_duplicates_indexs:
                        value = str(int(kmer_duplicates_values[index_dup_value])+int(readed_value))
                        index_dup_value += 1
                    else:
                        value = str(readed_value)
                else:
                    if index_column in new_kmer_indexs:
                        value = str(0)
                        index_new_value+=1
    ...
    def get_dupl_kmer_value_from_dict(self, kmer_dict, kmer_duplicates_index):
        ic = 0
        readed_value = ""
        l = list()
        self.__new_out_file.seek(0, 0)
        while True:
            ...
            if not c or c == b'\r' or c == b'\n':
                if ic != 0:
                    if ic in kmer_duplicates_index:
    ...
    def __get_duplicates_key_from_file(self, kmer_keys):
        self.__new_out_file.seek(0, 0)
        readed_value = ""
        while True:
            c = self.__new_out_file.read(1)
